...
subtest 'BasicAttribute Encode/Decode' => sub {
  ...
  ok(defined($ba2) && $ba2->isa('Game::EvonyTKR::Model::BasicAttribute'),
...
subtest 'Buff Encode/Decode' => sub {
  ...
  ok(defined($o2) && $o2->isa('Game::EvonyTKR::Model::Buff'), 'Decoding Buff');
...
subtest 'Book Encode/Decode' => sub {
  ...
  my $o = Game::EvonyTKR::Model::Book->from_wire_hash({
    _roles => ['Game::EvonyTKR::Model::Role::Book::Builtin',],
    name   => 'Attack Book',
  ...
  my $o2 = $decoder->decode($data);
  ok(defined($o2) && $o2->isa('Game::EvonyTKR::Model::Book'), 'Decoding Book');
...
subtest 'Specialty Encode/Decode' => sub {
  ...
  my ($file) =
    $dir->list->grep(sub { $_ =~ /\.ya?ml$/ && -f -r $_ })->head(1)->each;
  ...
  ok(
    $o->isa('Game::EvonyTKR::Model::Specialty'),
    'Creating Specialty with from_hash'
  );
  ...
  ok(
    $o2->isa('Game::EvonyTKR::Model::Specialty'),
    'Creating Specialty with from_wire_hash'
  );
  ...
  ok(defined($o3) && $o3->isa('Game::EvonyTKR::Model::Specialty'),
    'Decoding Specialty $o');

  my $o4 = $decoder->decode($data2);
  ok(defined($o4) && $o4->isa('Game::EvonyTKR::Model::Specialty'),
...
subtest 'Basic General Encode/Decode' => sub {
  ...
  my @files = $dir->list->grep(sub { $_ =~ /\.ya?ml$/ && -f -r $_ })->each;
  ...
  foreach my $file (@files) {
    ...
    ok(
      defined($object) && $object->isa('Game::EvonyTKR::Model::General'),
      sprintf('Instantiated General %s', $hashObject->{name})
    );
