...
foreach my $index (0..$#yaml_files) {
  ...
  if($general) {
    ...
    $logger->info(sprintf('successfully imported "%s"', $general->normalize($general->name)));
    eval {
      ...
      if($thawed->isa('Game::EvonyTKR::Model::General')){
        if($thawed->builtInBook->isa('Game::EvonyTKR::Model::Book')){
          ...
          $logger->logcroak('$thawed->builtInBook is not a Game::EvonyTKR::Model::Book');
        }
      } else {
    ...
    eval {
      ...
      $logger->debug(sprintf('setResult for %s: %s (defined: %s)',
        $key,
        $setResult // 'undef',
      ...
      $logger->info(sprintf('setResult: %s; retrieved and thawed general: %s',
...
is(scalar keys $generals->%*, scalar @yaml_files,
   "All general YAML files imported: " . scalar(keys $generals->%*) . " of " . scalar(@yaml_files));
# Validate structure
my @bad;
...
sub get_all_items($self, $client = undef) {
  ...
  if (defined $keys_data) {
    ...
    if (defined $individual) {
      $logger->info(
        "get_all_items Individual type: " . ref($individual));
      $logger->info("get_all_items Individual blessed: "
          . (blessed($individual) // 'NO'));
    }
