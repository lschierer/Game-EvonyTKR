...
diag("Working debuff - buffConditions: "
    . join(', ', @{ $working_debuff->buffConditions }));
diag("Working debuff - debuffConditions: "
    . join(', ', @{ $working_debuff->debuffConditions }));

my $working_matcher =
  Game::EvonyTKR::Model::Buff::Matcher->new(toTest => $working_debuff);
...
my $failing_debuff = Game::EvonyTKR::Model::Buff->new(
  ...
  ,    # Different condition that might not match
  debuffConditions => ['Enemy'],        # Same debuff condition
  targetedType     => 'Ground Troops'
);

diag("Failing debuff - buffConditions: "
    . join(', ', @{ $failing_debuff->buffConditions }));
diag("Failing debuff - debuffConditions: "
    . join(', ', @{ $failing_debuff->debuffConditions }));

my $failing_matcher =
  Game::EvonyTKR::Model::Buff::Matcher->new(toTest => $failing_debuff);
...
my $corrected_result = $failing_matcher->match(
  ...
  'Ground Troops',    # test_tt
  ['Against Monsters']
  ,                   # testBuffs (matching the debuff's actual buff conditions)
  ['Enemy'],          # testDebuffs
...
if ($corrected_result) {
  ...
  diag(
    "STILL BROKEN: Ascending-style debuff failed even with correct conditions");
}
