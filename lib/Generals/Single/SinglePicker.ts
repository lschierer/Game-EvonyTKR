...
export class PairPicker extends LitElement {
  ...
  override connectedCallback(): void {
    ...
    if (this.data) {
      this.data.generalStore.subscribe(() => {
        ...
        for (const entry of this.data.generalStore.getCatalog()) {
          primaries.add(entry.primary);
        }
        this.data.buffFilter.setAvailable([...primaries]);
        this.requestUpdate();
      });

      void this.data.generalStore.updateCatalog();
    }
  }
  ...
  private selectFiltered = (primary: string, newState: boolean) => {
    ...
    if (willSelect !== wasSelected) {
      ...
      this.data.buffFilter.toggle(primary);
      this.data.generalStore.toggleAllIgnoredForPrimary(primary);
    }
  };
  ...
  protected renderGeneralFilter() {
    ...
    if (this.data) {
      ...
      const filteredPrimaries = this.data.buffFilter
        .getAvailable()
        .filter(
          (primary) =>
      ...
      for (const primary of filteredPrimaries) {
        const itemSelected = this.data.buffFilter.isSelected(primary);
