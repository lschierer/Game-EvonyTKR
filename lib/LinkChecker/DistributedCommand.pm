...
has 'logger->file' => sub {
  return Path::Tiny::path(File::HomeDir::Tiny::home)
    ->child('var/log/Perl/dist/')
    ->child(__PACKAGE__)
    ->child('linkchecker_access.log');
};
has 'max_retries'      => 3;
...
sub init ($self) {
  ...
  if ($self->logger->file =~ m{^(.+)/[^/]+$}) {
    ...
    unless (-d $logger->dir) {
      mkdir $logger->dir
        or warn "Could not create log directory $logger->dir: $!";
    }
  }
...
sub _logger->access($self, $url, $status, $duration){
  ...
  while (%pending_internal || %pending_external || @active_jobs) {
    ...
    @active_jobs = grep {
      ...
      if ($info->{state} eq 'inactive') {
        ...
        if ($queue_time > $self->inactive_timeout) {
          ...
          $job->fail(
            "Job stuck inactive for ${queue_time}s - workers may have died");
      ...
      if (exists $job_start_times{$job_id} && $info->{state} eq 'active') {
        ...
        if ($elapsed > $self->job_timeout) {
          say
            "Job $job_id timed out after ${elapsed}s of execution, failing it";
    ...
    my $has_pending_internal =
      grep { $_ == 0 || $_ == -1 } values %pending_internal;
    my $has_pending_external =
      grep { $_ == 0 || $_ == -1 } values %pending_external;
    ...
    if ($now - $last_status_time > $stall_check_interval) {
      ...
      my $pending_int_count =
        scalar(grep { $_ == 0 || $_ == -1 } values %pending_internal);
      my $pending_ext_count =
        scalar(grep { $_ == 0 || $_ == -1 } values %pending_external);

      say sprintf(
"Status: %d checked, %d pending internal, %d pending external, %d active jobs",
        $current_checked,   $pending_int_count,
        $pending_ext_count, scalar(@active_jobs)
      );
      ...
      if ($current_checked == $last_progress_count && @active_jobs > 0) {
        ...
        if ($live_workers == 0 && @active_jobs > 0) {
          say
"ERROR: All workers died, failing remaining jobs to exit gracefully";
        ...
        for my $job_id (@active_jobs) {
          ...
          my $info = $job->info;
          my $queue_time =
            exists $job_enqueue_times{$job_id}
            ? time() - $job_enqueue_times{$job_id}
            : 'unknown';
...
sub _process_urls ($self, $job, $urls, $depth, $base_host) {
  ...
  for my $url_with_fragment (@$urls) {
    ...
    my $start_time = time();
    my ($status, $size) =
      $self->_check_url_with_metrics($url_for_request, $base_host);
