...
# that also means that it sometimes does not set the debuff conditions correctly.
    ...
    foreach my $cond (@$conditions_list) {
      ...
      if ($cond eq 'enemy') {
        ...
        $self->log_debug("Added buff condition: $cond ; set_condition result: $r");
      }
      else {
  ...
  sub generate_grammar ($self) {
    ...
    } keys %{ $self->MappedConditionNames };
