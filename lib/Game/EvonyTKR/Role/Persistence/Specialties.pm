...
sub add_specialty ($self, $specialty) {
  ...
  $self->persistence->store_specialty($normalized_name, $specialty->to_wire_hash());
  return 1;
}
...
sub get_specialty ($self, $name) {
  ...
  __PACKAGE__, $name, $normalized_name, defined($wire_data) ? 'successfully' : 'unsucessfully',
  ));
  ...
  unless($specialty){
    $roleLogger->error(sprintf('failed to hydrate "%s" using factory', $wire_data->{name}));
  }
  return $specialty;
}
