...
class Game::EvonyTKR::Model::General::Pair::Manager :isa(Game::EvonyTKR::Model::Data) {
  ...
  method build_pairs() {
    ...
    foreach my $primary (values %{ $generals } ) {
      foreach my $secondary (values %{ $generals } ) {
        ...
        $self->logger->debug(sprintf('testing if %s and %s conflict.', $primary->name , $secondary->name));
        next unless $conflicts->are_generals_compatible($primary->name, $secondary->name);

        $self->logger->debug(sprintf('no conflict, testing %s and %s for common type.', $primary->name , $secondary->name));
        ...
        @common = sort @common;
        next unless @common;
        $self->logger->debug(sprintf('%s and %s pair based on %s', $primary->name , $secondary->name, Data::Printer::np(@common)));
