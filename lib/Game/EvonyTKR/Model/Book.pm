...
sub addBuff ($self, $newBuff) {
  ...
  $self->logger->debug(sprintf(
    'Adding buff of class "%s" to book "%s"', $classList, $self->name));
  ...
  if (scalar @{ $self->buffs }) {
    for my $b (@{ $self->buffs }) {
      ...
      if (not($bcl[1] eq 'EvonyTKR' and $bcl[2] eq 'Buff')) {
        push @errors,
          sprintf('$buff must contain type Game::EvonyTKR::Model::Buff not %s',
          $bc);
      }
    }
  }
  unless (not Scalar::Util::looks_like_number($self->name)) {
    push @errors => sprintf('$name must contain a string, not %s', $self->name);
  }
  unless (not Scalar::Util::looks_like_number($self->text)) {
    push @errors => sprintf('$text must contain a string, not %s', $self->text);
  }
  if (@errors) {
...
sub to_hash ($self) {
  ...
  $hash->{_roles} = [
    $self->can('is_builtin')
      && $self->is_builtin == 1 ? 'Game::EvonyTKR::Model::Role::Book::Builtin'
    : (),
