...
class Game::EvonyTKR::Model::Buff::Matcher :isa(Game::EvonyTKR::Model::Data) {
  ...
  method matchTargetedType($test_tt, $logID) {
    if(scalar @{ $toTest->targetedTypes} ) {
      ...
      if (none { $_ =~ /\Q$test_tt\E/i }
            @{$toTest->targetedTypes }) {
        $self->logger->debug($logID . sprintf(
          '  ✗ Rejected: targetedType "%s" not matched by %s',
          $test_tt,
          Data::Printer::np($toTest->targetedTypes)
        ));
        return 0;
      }
    } else {
  ...
  method matchDebuffConditions ($testDebuffs, $logID) {
    ...
    if ($has_debuff_conditions) {
      ...
      $self->logger->debug("past check for no debuff conditions. " . scalar @$testDebuffs);
  ...
  method matchBuffConditions ($testBuffs, $logID) {
    if( scalar @{ $toTest->buffConditions }) {
      if(scalar @{ $testBuffs } == 0 ) {
        $self->logger->debug($logID . '  ✗ Rejected: This buff has conditions, and none are allowed.');
        return 0;
      }
  ...
  method match ($test_attribute, $test_tt, $testBuffs, $testDebuffs , $logID) {
    ...
    $self->logger->debug(sprintf("$logID Matcher called with: attr=%s, tt=%s, buffs=%s, debuffs=%s",
    ...
    if (scalar @$testBuffs == 0) {
      ...
      $self->logger->debug("$logID Empty buff conditions provided, using defaults instead");
    }
    if(length($test_tt)){
      ...
      if(!$self->matchDebuffConditions($testDebuffs, $logID)){
        $self->logger->debug("$logID Rejecting based on "
        . join(', ', @{$testDebuffs} ));
        return 0;
      }
