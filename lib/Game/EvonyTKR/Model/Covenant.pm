...
sub from_hash($class, $object, $primary_general = undef) {
  ...
  $logger->debug(sprintf("from_hash: primary_general param is %s",
...
sub from_wire_hash ($class, $h) {
  ...
  unless (defined($primary)
    && ref($primary)
    && $primary->isa('Game::EvonyTKR::Model::General')) {
    $logger->error(sprintf('Could not find general for covenant with name %s',
      $h->{primary}));
    return;
  }
  $logger->debug(sprintf(
    'found primary general for "%s", starting import.', $h->{primary}));
  $logger->debug(sprintf(
