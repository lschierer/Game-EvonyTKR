  ...
  method init() {
    ...
    while (my $file = glob(File::Spec->catfile($data_location, '*.yaml'))) {
      ...
      if( $filename eq $cgData->{'name'}){
        ...
        $self->logger()->error(sprintf('%s has name %s which does not match.',
      ...
      $self->logger()->debug(sprintf('members are %s, I read in %s.',
      ...
      for my $member (@members) {
       ...
       if(defined $general) {
        ...
        if(index($classList, 'Game::EvonyTKR::General') != -1) {
          ...
          $self->logger()->error(sprintf('Model returned some %s not a general.',
          $classList));
        }
       }
       else {
        $self->logger()->warn(sprintf(
          'Model could not find general %s', $member));
       }
      }

      my @books = @{ $cgData->{'books'}};
      $self->logger()->debug(sprintf('books are %s, I read in %s.',
      ...
      $self->logger()->debug(sprintf('others are %s, I read in %s.',
