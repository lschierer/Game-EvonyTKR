...
sub simulate ($self, $params) {
  ...
  my $base_defense = $self->reference_data->get_base_defense($tier, $troop_type);
  ...
  my $player_attack  = $base_attack * (1 + $total_attack_buff_pct) + $attack_buff_flat;
  my $player_defense = $base_defense * (1 + $total_defense_buff_pct) + $defense_buff_flat;
  my $player_hp      = $base_hp * (1 + $total_hp_buff_pct) + $hp_buff_flat;

  # Get troop modifier based on monster type
  my $troop_modifier = $self->_calculate_troop_modifier(
    $monster, $tier, $troop_type, $params->{alliance_boss_modifier}
  );
  ...
  my $effective_damage = $damage_per_troop * ($player_attack / ($player_attack + $monster_defense));

  # Calculate minimum troops to kill (simplified)
  my $min_troops_to_kill = 0;
  if ($effective_damage > 0 && $monster->troop_count) {
    $min_troops_to_kill = int(
      ($monster_hp * $monster->troop_count) / $effective_damage
    ) + 1;
  }

  # Calculate expected wounds (simplified)
  # Wounds depend on monster attack vs player defense/hp
  my $monster_damage_per_troop = $monster_attack / $troop_count if $troop_count > 0;
  my $expected_wounds = 0;
  if ($player_hp > 0 && $monster_attack > 0) {
    $expected_wounds = int(
      ($monster_attack * $monster->troop_count) / $player_hp
    );
  }
...
sub _calculate_troop_modifier ($self, $monster, $tier, $troop_type, $alliance_modifier = undef) {
  my $order = $monster->order;

  # Check for world boss (order 321-324)
  if ($self->is_world_boss($order)) {
    return $self->reference_data->get_world_boss_modifier_by_order($order, $troop_type);
  }

  # Check for alliance boss (order 693-695)
  if ($self->is_alliance_boss($order)) {
    my $base_mod = $self->reference_data->get_alliance_boss_modifier($tier, $troop_type);
