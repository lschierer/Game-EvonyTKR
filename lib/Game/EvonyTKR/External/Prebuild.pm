...
package Game::EvonyTKR::External::Prebuild {
  ...
  sub log_config ($self, $app) {
    $logger = sub { return $app->get_logger(__PACKAGE__); }
  }
  ...
  sub register ($self, $app, $conf = {}) {
    ...
    $app->plugins->on(
      generals_loaded => sub ($plugin, $data) {
        my $generals = $data->{generals};
        unless ($generals && ref($generals) eq 'HASH') {
          $logger->ERR(sprintf(
'in %s generals_loaded callback, generals is a "%s" rather than a HASH',
       ...
       # send a list of names to the workers and reconstruct the generals there.
        my @general_names =
          map { $_->name } sort { $a->name cmp $b->name } values $generals->%*;
    ...
    $app->minion->add_task(
      PairBuilder_kickoff => sub ($job, $args) {
        __PACKAGE__->log_config($job->app);
        ...
        unless ($generals && ref($generals) eq 'ARRAY') {
          $logger->ERR(sprintf(
'PairBuilder_kickoff requires a general_names of type ARRAY not "%s"',
            defined($generals) ? ref($generals) : 'Undefined'));
