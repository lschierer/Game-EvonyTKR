...
qr/elementor-element-(?:\w){1,9}.elementor-widget.elementor-widget-theme-post-content/
    ...
    while ($h4_index <= $#headers) {
      if ($headers[$h4_index]->tag eq 'h4') {
        ...
        if ($para) {
          ...
          $self->logger->ERR("Could not find the required paragraph");
        }
        last;
      }
      else {
    ...
    unless (length($name) && length($text)) {
      $self->logger->ERR("Cannot find and parse the required H3 tag!!");
    }
...
qr/elementor-element-(?:\w){1,9}.elementor-widget.elementor-widget-theme-post-content/
    ...
    unless (length($name)) {
      $self->logger->ERR("Cannot find the required H3 tag!!");
    }
  }
