  ...
  sub merge_into_pairs_by_type($self, $pairs_by_type, @new_pairs){
    foreach my $np (@new_pairs){
      unless (List::AllUtils::any { $_ eq $np->{type} } $self->GeneralKeys->@* ){
        $self->logger->error(sprintf('pair type must be one of %s, not %s',
        join(', ', $self->GeneralKeys->@*), $np->{type} ));
        next;
      }
      $pairs_by_type->{ $np->{type} } //= [];
      $pairs_by_type->{ $np->{type} } = [ List::UtilsBy::uniq_by { sprintf('%s/%s/%s',
      $_->{type}, $_->{primary}, $_->{secondary}) } ($np, $pairs_by_type->{ $np->{type} }->@* ) ];
    }
