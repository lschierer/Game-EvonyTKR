...
package Game::EvonyTKR::Plugins::Generals::Pairs {
    ...
    $app->plugins->on('evonytkrtips_initialized' => sub($self, $manager){
      ...
      foreach my $type ( @generalTypes ) {
        ...
        $routes->get("/$linkTarget/")
          ->to(controller => $controller_name, action => 'typeIndex', generalType => $type, linkTarget => $linkTarget)
  ...
  sub typeIndex ($self) {
      ...
      my $sort_param = $self->param('sort') // 'primary,secondary,marchbuff';  # Default sort columns
      my $dir_param = $self->param('dir') // 'desc,desc,desc';            # Default directions
      ...
      my %sort_spec = map { $sort_columns[$_] => $sort_dirs[$_] } 0..$#sort_columns;

      $logger->debug("Sorting by: " . Data::Printer::np(%sort_spec));

      # Get pairs from manager
      my $pm = $self->app->get_general_pair_manager();
      my @pairs = @{ $pm->get_pairs_by_type($generalType) };
      my @processed = (0) x scalar(@pairs);
      MCE::Flow->init(
        max_workers => 'auto',
        chunk_size => 1
      );
      MCE::Flow->run(sub ($mce, $chunk_ref, $chunk_id) {
        my $idx = $chunk_ref->[0];
        $logger->debug("chunk_id is $chunk_id, idx is $idx");
        if(not $pairs[$idx]->rootManager ) {
          $pairs[$idx]->setRootManager($self->app->get_root_manager() );
        }
        $pairs[$idx]->updateBuffs();
        $processed[$idx] = 1;
        return 1;
      }, 0..scalar(@pairs));

      unless (all { $_ == 1 } @processed) {
        $logger->warn("Some pairs were not processed during parallel execution");
      }
      ...
      @pairs = sort {
          ...
          for my $i (0..$#sort_columns) {
              ...
              if ($col eq 'primary') {
                ...
                $cmp = $a->marchbuff cmp $b->marchbuff;
              }
    ...
    my $markdown_path = $distDir->child("pages/generals/pairs/$linkTarget/index.md");
