...
"getEvAnsScore for $name recieved $result from getEvAnsScore for buff $i"
        ...
        $self->logger()
          ->debug("getEvAnsScore for $name; $category currently has value: "
            . $resultRef->{'AES'}->{$category});
        $resultRef->{$category}->{'AES'} += $result;
        $self->logger()
          ->trace(
          "getEvAnsScore for $name; $category -> AES  currently has value: "
            . $resultRef->{$category}->{'AES'});
  ...
  method toHashRef($verbose = 0) {
    $self->logger()
      ->trace("Starting toHashRef for Ascending, verbose is $verbose");
    my $returnRef = {};
  ...
  method readFromFile($name) {
    ...
    if (-T -s -r $FileWithPath) {
      $self->logger()
        ->debug("AscendingFile $AscendingFileName exists as expected");
      ...
      foreach my $fl (@fileLevels) {
        ...
        foreach my $flb (@flBuffs) {
          ...
          if (any { $_ eq 'value' } @flKeys) {
            ...
            $self->logger()
              ->warn("$AscendingFileName has a buff without a value");
          }
          if (defined $b) {
            ...
            $self->logger()
              ->info(
              "Adding buff from $AscendingFileName to " . $fl->{'level'});
