...
use lib "$FindBin::Bin/../../PAGI-WebServer/lib";  # WebFramework modules
use Getopt::Long qw( GetOptions );
use List::AllUtils qw( any uniq max min sum );
use Scalar::Util qw( refaddr );
use Path::Tiny;

# Use the new Thunderhorse-style loader directly
use Game::EvonyTKR::Loader::Generals;
use Game::EvonyTKR::Loader::Books;
use Game::EvonyTKR::Role::Common;
...
# Create a simple object with the normalize method
package FeatureExtractor {
  use Moo;
  with 'Game::EvonyTKR::Role::Common';
}

my $extractor = FeatureExtractor->new();

# Load books first (generals need them for builtInBook)
say STDERR "Loading books...";
my $books_loader = Game::EvonyTKR::Loader::Books->new(
  data_dir => "$FindBin::Bin/../share/collections/data"
);
$books_loader->load_all();
say STDERR "Loaded " . scalar(keys %{ $books_loader->skill_books }) . " skill books (builtin)";
say STDERR "Loaded " . scalar(keys %{ $books_loader->generic_books }) . " generic books";

# Load generals using the new loader
say STDERR "Loading generals...";
my $generals_loader = Game::EvonyTKR::Loader::Generals->new(
  data_dir => "$FindBin::Bin/../share/collections/data/generals"
);
$generals_loader->load_all();

my @generals;
my %general_by_name;

for my $key (@{ $generals_loader->list_generals() }) {
  my $general = $generals_loader->get_general($key);
  next unless $general;

  # Populate builtin book from the books loader (skill books = builtin books)
  my $book_name = $general->builtInBookName;
  if ($book_name) {
    my $book = $books_loader->get_skill_book($book_name);
    if ($book) {
      $general->builtInBook($book);
    }
    else {
      warn "Could not find skill book '$book_name' for general " . $general->name . "\n";
    }
  }

  push @generals, $general;
  my $nn = lc($extractor->normalize($general->name));
...
my @feature_names = (
  ...
  'g1_ground_specialist',
  'g1_siege_specialist',
  'g2_mounted_specialist',
  'g2_ranged_specialist',
  'g2_ground_specialist',
  'g2_siege_specialist',
  'same_specialist_type',
  # Group structure features
  'g1_multi_attr_groups',
  'g2_multi_attr_groups',
  'g1_solo_buffs',
  'g2_solo_buffs',
  'g1_total_buffs',
  'g2_total_buffs',
...
sub extract_training_data ($fh, $general_by_name) {
  ...
  my $conflict_file = "$FindBin::Bin/../share/training_data/conflicting_pairs";
  if (-f $conflict_file) {
    open my $conflict_fh, '<', $conflict_file
      or die "Cannot open $conflict_file: $!\n";
    while (my $line = <$conflict_fh>) {
      ...
      my $g1 = $general_by_name->{ lc($extractor->normalize($g1_name)) };
      my $g2 = $general_by_name->{ lc($extractor->normalize($g2_name)) };
      next unless $g1 && $g2;
  ...
  my $working_count = 0;
  my @pair_files =
    grep { $_ !~ /conflicting_pairs$/ }
    glob("$FindBin::Bin/../share/training_data/*_pairs");
...
sub extract_features ($g1, $g2) {
  ...
  # General names
  push @features, lc($extractor->normalize($g1->name)),
    lc($extractor->normalize($g2->name));
  ...
  # Group structure - check if builtInBook exists
  my $g1_book = $g1->builtInBook;
  my $g2_book = $g2->builtInBook;

  my ($g1_groups, $g2_groups) = ([], []);
  my ($g1_multi,  $g2_multi)  = (0,  0);
  my ($g1_solo,   $g2_solo)   = (0,  0);
  my (@g1_buffs,  @g2_buffs)  = ((), ());

  if ($g1_book && $g1_book->buffs) {
    $g1_groups = find_groups($g1_book);
    $g1_multi  = count_multi_attr_groups($g1_groups);
    $g1_solo   = count_solo_buffs($g1_book, $g1_groups);
    @g1_buffs  = grep { !$_->passive } @{ $g1_book->buffs };
  }

  if ($g2_book && $g2_book->buffs) {
    $g2_groups = find_groups($g2_book);
    $g2_multi  = count_multi_attr_groups($g2_groups);
    $g2_solo   = count_solo_buffs($g2_book, $g2_groups);
    @g2_buffs  = grep { !$_->passive } @{ $g2_book->buffs };
  }
  ...
  # Condition features
  my ($both_attacking, $both_defending, $both_monster) =
    compare_conditions($g1, $g2);
  ...
  # Book text complexity
  my $g1_text_len = $g1_book ? length($g1_book->text // '') : 0;
  my $g2_text_len = $g2_book ? length($g2_book->text // '') : 0;
...
sub count_solo_buffs ($book, $groups) {
  return 0 unless $book && $book->buffs;
  my @buffs = grep { !$_->passive } @{ $book->buffs };
  my %grouped;
...
sub compare_attribute ($g1, $g2, $attr) {
  ...
  return (0, 0, 0) unless $g1_book && $g2_book;
  return (0, 0, 0) unless $g1_book->buffs && $g2_book->buffs;

  my @g1_buffs =
    grep { $_->attribute eq $attr && !$_->passive } @{ $g1_book->buffs };
  my @g2_buffs =
    grep { $_->attribute eq $attr && !$_->passive } @{ $g2_book->buffs };
  ...
  for my $t (keys %g1_types) {
    $overlap = 1
      if exists $g2_types{$t}
      || $t eq 'global'
      || (grep { $_ eq 'global' } keys %g2_types);
  }
...
sub compare_conditions ($g1, $g2) {
  ...
  my @g1_buffs = grep { !$_->passive } @{ $g1_book->buffs };
  my @g2_buffs = grep { !$_->passive } @{ $g2_book->buffs };

  my $g1_attacking =
    any { my @c = @{ $_->conditions // [] }; any { /Attacking/i } @c }
  @g1_buffs;
  my $g2_attacking =
    any { my @c = @{ $_->conditions // [] }; any { /Attacking/i } @c }
  @g2_buffs;
  ...
  my $g1_defending =
    any { my @c = @{ $_->conditions // [] }; any { /Defending/i } @c }
  @g1_buffs;
  my $g2_defending =
    any { my @c = @{ $_->conditions // [] }; any { /Defending/i } @c }
  @g2_buffs;
  ...
  my $g1_monster =
    any { my @c = @{ $_->conditions // [] }; any { /Monster/i } @c } @g1_buffs;
  my $g2_monster =
    any { my @c = @{ $_->conditions // [] }; any { /Monster/i } @c } @g2_buffs;
...
sub count_condition_overlap ($g1, $g2) {
  ...
  return 0 unless $g1_book && $g2_book;
  return 0 unless $g1_book->buffs && $g2_book->buffs;
